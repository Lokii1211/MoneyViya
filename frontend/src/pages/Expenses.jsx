...
export default function Expenses() {
  ...
  const load = async () => {
    setLoading(true)
    const t = await api.getTransactions(phone)
    setTxns(t || [])
    if (t && t.length > 10) {
      const cats = {}
  ...
  const submit = async () => {
    ...
    toast.show(type === 'expense' ? 'Expense added!' : 'Income recorded!', 'success')
    load()
  }
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (parsed.error) {
        toast.show('Could not read the bill. Try a clearer image.', 'warning')
        setShowOCR(false)
        ...
        toast.show('Could not detect amount. Add manually.', 'warning')
        setShowOCR(false)
      }
    } catch (err) {
      console.error('OCR error:', err)
      toast.show('OCR failed. Add manually.', 'error')
      setShowOCR(false)
    }
    setOcrLoading(false)
  }
  ...
  const confirmOCR = async () => {
    ...
    toast.show(`₹${ocrResult.amount} ${ocrResult.type} added from bill!`, 'success')
  ...
  const handleSMSParse = () => {
    ...
    if (result) {
      ...
      toast.show('Could not parse this message. Try a bank debit SMS.', 'error')
    }
  }
  ...
  const todayExpenses = txns.filter(t => {
    ...
    return new Date(t.created_at).toLocaleDateString() === new Date().toLocaleDateString()
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div className="flex gap-2">
          <button className="btn-secondary ripple" style={{ padding: '8px 12px', fontSize: 13, borderRadius: 10 }} onClick={() => fileRef.current?.click()}>
            <Camera size={15} style={{ marginRight: 4 }}/> Scan Bill
          </button>
          <button className="btn-secondary ripple" style={{ padding: '8px 12px', fontSize: 13, borderRadius: 10 }} onClick={() => setShowSMS(!showSMS)}>
            📱 SMS
          </button>
      ...
      {loading && !txns.length ? (
        ...
        <>
          {/* Money Left Today */}
          <div className="stat-card" style={{ borderRadius: 18, padding: '24px 20px', marginBottom: 16, textAlign: 'center', background: 'linear-gradient(135deg, var(--primary-dim), var(--cyan-dim))' }}>
            <div className="stat-label" style={{ letterSpacing: 2 }}>MONEY LEFT TODAY</div>
            <div className="stat-value" style={{ fontSize: 42, fontWeight: 900, color: moneyLeft >= 0 ? 'var(--primary)' : 'var(--red)' }}>
              ₹{animatedMoneyLeft}
            </div>
            {moneyLeft < 0 && <div style={{ fontSize: 12, color: 'var(--red)', fontWeight: 700, marginTop: 4 }}>⚠️ Over budget by ₹{Math.abs(moneyLeft)}</div>}
            <div className="body-s text-secondary" style={{ marginTop: 6 }}>Daily budget: ₹{dailyBudget} · Spent: ₹{todayExpenses}</div>
          </div>
          ...
          <div className="stat-grid">
            <div className="stat-card">
              ...
              <div className="stat-value" style={{ color: 'var(--primary)' }}>₹{animatedIncome}</div>
            </div>
            <div className="stat-card">
              <div className="flex items-center gap-2" style={{ marginBottom: 4 }}>
          ...
          <div className="home-section">
            <div className="stat-label" style={{ marginBottom: 6 }}>⚡ QUICK ADD EXPENSE</div>
            <div className="pill-bar">
              {QUICK_AMOUNTS.map(a => (
                <button key={a} className="pill-btn" onClick={() => { setAmount(String(a)); setShowAdd(true) }}>
                  ₹{a}
                </button>
              ))}
            </div>
          ...
          {showOCR && (
            <div className="entry-form" style={{ marginBottom: 16, borderColor: 'var(--violet)' }}>
              {ocrLoading ? (
                <div className="text-center" style={{ padding: 20 }}>
                  <div style={{ fontSize: 32, marginBottom: 8 }}>🔍</div>
                  <div style={{ fontSize: 14, fontWeight: 700 }}>Scanning bill...</div>
                  <div className="body-s text-secondary">AI is reading your receipt</div>
                </div>
              ) : ocrResult ? (
                <>
                  ...
                  <div className="info-row" style={{ flexDirection: 'column', gap: 8, padding: 14, marginBottom: 12 }}>
                    <div className="flex justify-between w-full">
                      <span className="body-s text-tertiary">Amount</span>
                      <input type="number" value={ocrResult.amount} onChange={e => setOcrResult({...ocrResult, amount: Number(e.target.value)})}
                        className="form-input" style={{ background:'none', border:'none', fontSize:20, fontWeight:800, textAlign:'right', width:120, padding:0 }} />
                    </div>
                    <div className="flex justify-between w-full">
                      <span className="body-s text-tertiary">Type</span>
                      <div className="flex gap-1">
                        {['expense','income'].map(t => (
                          <button key={t} onClick={() => setOcrResult({...ocrResult, type: t})}
                            className={`pill-btn ${ocrResult.type === t ? 'active' : ''}`}
                            style={{ padding: '4px 10px', fontSize: 11 }}>{t}</button>
                        ))}
                      </div>
                    </div>
                    <div className="flex justify-between w-full">
                      <span className="body-s text-tertiary">Category</span>
                      <span style={{ fontSize: 13, fontWeight: 700 }}>{ocrResult.category}</span>
                    </div>
                    {ocrResult.description && (
                      <div className="flex justify-between w-full">
                        <span className="body-s text-tertiary">Description</span>
                        <span className="body-s text-secondary">{ocrResult.description}</span>
                      </div>
                    )}
                  </div>
                  <div className="form-actions">
                    ...
                    <button className="btn-primary" style={{ flex: 2 }} onClick={confirmOCR}>
          ...
          {showSMS && (
            <div className="entry-form" style={{ marginBottom: 16, borderColor: 'var(--cyan)' }}>
              <div className="flex items-center gap-2" style={{ marginBottom: 12 }}>
                <span style={{ fontSize: 20 }}>📱</span>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 800 }}>Paste Bank SMS</div>
                  <div className="body-s text-tertiary">Auto-detect amount & category from bank messages</div>
                </div>
              </div>
              <textarea value={smsText} onChange={e => setSmsText(e.target.value)}
                placeholder={'Paste your bank SMS here, e.g.:\nINR 450.00 debited from A/c XX1234 on 12-04-26 to SWIGGY'}
                className="form-input" style={{ minHeight: 80, resize: 'vertical', fontFamily: 'monospace' }}/>
              {smsResult && (
                <div className="info-row" style={{ flexDirection: 'column', gap: 6, padding: 14, marginTop: 10, marginBottom: 10 }}>
                  <div className="flex justify-between w-full">
                    <span className="body-s text-tertiary">Detected</span>
                    <span className="stat-value" style={{ fontSize: 18, color: smsResult.isIncome ? 'var(--primary)' : 'var(--cosmos-400)' }}>
                  ...
                  {smsResult.merchant && (
                    <div className="flex justify-between w-full">
                      <span className="body-s text-tertiary">Merchant</span>
                      <span className="body-s text-secondary">{smsResult.merchant}</span>
                    </div>
                  )}
                </div>
              )}
              <div className="form-actions" style={{ marginTop: 8 }}>
                <button className="btn-secondary" onClick={() => { setShowSMS(false); setSmsText(''); setSmsResult(null) }}>
                  <X size={14}/> Cancel
                </button>
                {!smsResult ? (
                  <button className="btn-primary" style={{ flex: 2 }} onClick={handleSMSParse} disabled={!smsText.trim()}>
                    🔍 Parse SMS
                  </button>
                ) : (
                  <button className="btn-primary" style={{ flex: 2 }} onClick={confirmSMS}>
          ...
          {subscriptions.length > 0 && !showAdd && !showSMS && !showOCR && (
            <div className="home-section">
              <div className="stat-label" style={{ marginBottom: 8 }}>🔄 RECURRING EXPENSES DETECTED</div>
              <div className="hscroll">
                {subscriptions.map((s, i) => (
                  <div key={i} className="stat-card" style={{ minWidth: 120, padding: '10px 12px' }}>
                    <div style={{ fontSize: 18, marginBottom: 4 }}>{s.icon}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 2 }}>{s.name}</div>
                    <div className="stat-value" style={{ fontSize: 13, color: 'var(--cosmos-400)' }}>₹{Math.round(s.total/s.count)}/avg</div>
                    ...
                    <button key={c} className={`cat-chip${category === c ? ' active' : ''}`} onClick={() => {
                      ...
                      if (amount && Number(amount) > 0) {
                        ...
                        fn(phone, Number(amount), c, note).then(() => {
                          setAmount(''); setNote(''); setShowAdd(false)
                          toast.show(type === 'expense' ? `₹${amount} ${c.split(' ')[1]} added!` : `₹${amount} income added!`, 'success')
                          load()
                        })
                      }
                    }}>{c}</button>
            ...
            txns.map(t => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button className="btn-ghost" style={{ opacity: 0.4, padding: 4 }} onClick={() => removeTxn(t.id)}><Trash2 size={14} /></button>
                </div>
