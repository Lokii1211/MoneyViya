// ===== VIYA i18n — Language Support (Hindi/Tamil/Telugu) =====
const strings = {
  en: {
    home: 'Home', money: 'Money', chat: 'Chat', habits: 'Habits', profile: 'Profile',
    greeting_morning: 'Good Morning', greeting_afternoon: 'Good Afternoon', greeting_evening: 'Good Evening', greeting_night: 'Good Night',
    money_left_today: 'MONEY LEFT TODAY', daily_budget: 'daily budget', over_budget: 'Over budget by',
    income: 'INCOME', expenses: 'EXPENSES', saved: 'SAVED',
    add_expense: 'Add Expense', briefing: 'Briefing', earn_more: 'Earn More', goals: 'Goals',
    report: 'Report', tax_save: 'Tax Save', plan_day: 'Plan Day',
    scan_bill: 'Scan Bill', add: 'Add', quick_add_expense: 'QUICK ADD EXPENSE',
    spent: 'SPENT', today: 'Today', yesterday: 'Yesterday',
    daily_habits: 'Daily Habits', streak: 'Streak', best: 'Best', done_today: 'Done Today',
    start_your_streak: 'Start Your Streak!', seven_day_streak: '7-DAY STREAK',
    savings_goals: 'Savings Goals', new_goal: 'New Goal', total_saved: 'TOTAL SAVED',
    settings: 'Settings', appearance: 'Appearance', notifications: 'Notifications',
    reminders: 'Reminders', redo_setup: 'Redo Setup', privacy: 'Privacy & Security',
    help: 'Help & Support', sign_out: 'Sign Out', language: 'Language',
    invite_friends: 'Invite Friends, Earn Rewards', family_mode: 'Family Mode',
    terms: 'Terms of Service',
    ask_viya: 'Ask Viya anything...', send: 'Send',
    loading: 'Loading...', no_data: 'No data yet', cancel: 'Cancel', save: 'Save', delete: 'Delete',
    confirm: 'Confirm', share: 'Share',
  },
  hi: {
    home: 'होम', money: 'पैसा', chat: 'चैट', habits: 'आदतें', profile: 'प्रोफाइल',
    greeting_morning: 'सुप्रभात', greeting_afternoon: 'नमस्ते', greeting_evening: 'शुभ संध्या', greeting_night: 'शुभ रात्रि',
    money_left_today: 'आज बचे पैसे', daily_budget: 'दैनिक बजट', over_budget: 'बजट से ज़्यादा',
    income: 'आय', expenses: 'खर्च', saved: 'बचत',
    add_expense: 'खर्च जोड़ें', briefing: 'ब्रीफिंग', earn_more: 'और कमाएं', goals: 'लक्ष्य',
    report: 'रिपोर्ट', tax_save: 'टैक्स बचाएं', plan_day: 'दिन प्लान करें',
    scan_bill: 'बिल स्कैन', add: 'जोड़ें', quick_add_expense: 'जल्दी खर्च जोड़ें',
    spent: 'खर्च', today: 'आज', yesterday: 'कल',
    daily_habits: 'दैनिक आदतें', streak: 'स्ट्रीक', best: 'सर्वश्रेष्ठ', done_today: 'आज पूरा',
    start_your_streak: 'अपनी स्ट्रीक शुरू करें!', seven_day_streak: '7-दिन स्ट्रीक',
    savings_goals: 'बचत लक्ष्य', new_goal: 'नया लक्ष्य', total_saved: 'कुल बचत',
    settings: 'सेटिंग्स', appearance: 'दिखावट', notifications: 'सूचनाएं',
    reminders: 'रिमाइंडर', redo_setup: 'दोबारा सेटअप', privacy: 'गोपनीयता',
    help: 'मदद और सपोर्ट', sign_out: 'साइन आउट', language: 'भाषा',
    invite_friends: 'दोस्तों को बुलाएं, इनाम पाएं', family_mode: 'फैमिली मोड',
    terms: 'सेवा की शर्तें',
    ask_viya: 'Viya से कुछ भी पूछो...', send: 'भेजें',
    loading: 'लोड हो रहा है...', no_data: 'अभी कोई डेटा नहीं', cancel: 'रद्द करें', save: 'सेव करें', delete: 'हटाएं',
    confirm: 'पक्का करें', share: 'शेयर करें',
  },
  ta: {
    home: 'முகப்பு', money: 'பணம்', chat: 'அரட்டை', habits: 'பழக்கங்கள்', profile: 'சுயவிவரம்',
    greeting_morning: 'காலை வணக்கம்', greeting_afternoon: 'மதிய வணக்கம்', greeting_evening: 'மாலை வணக்கம்', greeting_night: 'இரவு வணக்கம்',
    money_left_today: 'இன்று மீதம் உள்ள பணம்', daily_budget: 'தினசரி பட்ஜெட்', over_budget: 'பட்ஜெட் மீறியது',
    income: 'வருமானம்', expenses: 'செலவுகள்', saved: 'சேமிப்பு',
    add_expense: 'செலவு சேர்', earn_more: 'மேலும் சம்பாதி', goals: 'இலக்குகள்',
    report: 'அறிக்கை', scan_bill: 'பில் ஸ்கேன்', add: 'சேர்',
    spent: 'செலவு', today: 'இன்று', yesterday: 'நேற்று',
    daily_habits: 'தினசரி பழக்கங்கள்', streak: 'ஸ்ட்ரீக்', best: 'சிறந்த',
    savings_goals: 'சேமிப்பு இலக்குகள்', total_saved: 'மொத்த சேமிப்பு',
    sign_out: 'வெளியேறு', language: 'மொழி', terms: 'சேவை விதிமுறைகள்',
    invite_friends: 'நண்பர்களை அழை, பரிசு பெறு', family_mode: 'குடும்ப பயன்முறை',
    ask_viya: 'Viya-விடம் எதையும் கேளுங்கள்...', send: 'அனுப்பு',
    loading: 'ஏற்றுகிறது...', cancel: 'ரத்து', save: 'சேமி', share: 'பகிர்',
  },
  te: {
    home: 'హోమ్', money: 'డబ్బు', chat: 'చాట్', habits: 'అలవాట్లు', profile: 'ప్రొఫైల్',
    greeting_morning: 'శుభోదయం', greeting_afternoon: 'శుభ మధ్యాహ్నం', greeting_evening: 'శుభ సాయంత్రం', greeting_night: 'శుభ రాత్రి',
    money_left_today: 'ఈరోజు మిగిలిన డబ్బు', daily_budget: 'రోజువారీ బడ్జెట్', over_budget: 'బడ్జెట్ మించింది',
    income: 'ఆదాయం', expenses: 'ఖర్చులు', saved: 'పొదుపు',
    add_expense: 'ఖర్చు జోడించు', earn_more: 'ఇంకా సంపాదించు', goals: 'లక్ష్యాలు',
    report: 'రిపోర్ట్', scan_bill: 'బిల్ స్కాన్', add: 'జోడించు',
    spent: 'ఖర్చు', today: 'ఈరోజు', yesterday: 'నిన్న',
    daily_habits: 'రోజువారీ అలవాట్లు', streak: 'స్ట్రీక్', best: 'ఉత్తమ',
    savings_goals: 'పొదుపు లక్ష్యాలు', total_saved: 'మొత్తం పొదుపు',
    sign_out: 'సైన్ ఔట్', language: 'భాష', terms: 'సేవా నిబంధనలు',
    invite_friends: 'స్నేహితులను ఆహ్వానించు', family_mode: 'కుటుంబ మోడ్',
    ask_viya: 'Viya ని ఏదైనా అడగండి...', send: 'పంపు',
    loading: 'లోడ్ అవుతోంది...', cancel: 'రద్దు', save: 'సేవ్', share: 'షేర్',
  },
  kn: {
    home: 'ಮುಖಪುಟ', money: 'ಹಣ', chat: 'ಚಾಟ್', habits: 'ಅಭ್ಯಾಸಗಳು', profile: 'ಪ್ರೊಫೈಲ್',
    greeting_morning: 'ಶುಭೋದಯ', greeting_afternoon: 'ಶುಭ ಮಧ್ಯಾಹ್ನ', greeting_evening: 'ಶುಭ ಸಂಜೆ', greeting_night: 'ಶುಭ ರಾತ್ರಿ',
    money_left_today: 'ಇಂದು ಉಳಿದ ಹಣ', daily_budget: 'ದೈನಿಕ ಬಜೆಟ್', over_budget: 'ಬಜೆಟ್ ಮೀರಿದೆ',
    income: 'ಆದಾಯ', expenses: 'ಖರ್ಚುಗಳು', saved: 'ಉಳಿತಾಯ',
    add_expense: 'ಖರ್ಚು ಸೇರಿಸಿ', earn_more: 'ಹೆಚ್ಚು ಗಳಿಸಿ', goals: 'ಗುರಿಗಳು',
    report: 'ವರದಿ', scan_bill: 'ಬಿಲ್ ಸ್ಕ್ಯಾನ್', add: 'ಸೇರಿಸಿ',
    spent: 'ಖರ್ಚು', today: 'ಇಂದು', yesterday: 'ನಿನ್ನೆ',
    daily_habits: 'ದೈನಿಕ ಅಭ್ಯಾಸಗಳು', streak: 'ಸ್ಟ್ರೀಕ್', best: 'ಅತ್ಯುತ್ತಮ',
    savings_goals: 'ಉಳಿತಾಯ ಗುರಿಗಳು', total_saved: 'ಒಟ್ಟು ಉಳಿತಾಯ',
    sign_out: 'ಸೈನ್ ಔಟ್', language: 'ಭಾಷೆ', terms: 'ಸೇವಾ ನಿಯಮಗಳು',
    invite_friends: 'ಸ್ನೇಹಿತರನ್ನು ಆಹ್ವಾನಿಸಿ', family_mode: 'ಕುಟುಂಬ ಮೋಡ್',
    ask_viya: 'Viya ಅನ್ನು ಏನಾದರೂ ಕೇಳಿ...', send: 'ಕಳುಹಿಸಿ',
    loading: 'ಲೋಡ್ ಆಗುತ್ತಿದೆ...', cancel: 'ರದ್ದು', save: 'ಉಳಿಸಿ', share: 'ಹಂಚಿಕೊಳ್ಳಿ',
  }
}

export const LANGUAGES = [
  { code: 'en', label: 'English', native: 'English' },
  { code: 'hi', label: 'Hindi', native: 'हिंदी' },
  { code: 'ta', label: 'Tamil', native: 'தமிழ்' },
  { code: 'te', label: 'Telugu', native: 'తెలుగు' },
  { code: 'kn', label: 'Kannada', native: 'ಕನ್ನಡ' },
]

export function t(key) {
  const lang = localStorage.getItem('mv_lang') || 'en'
  return strings[lang]?.[key] || strings.en[key] || key
}

export function getLang() {
  return localStorage.getItem('mv_lang') || 'en'
}

export function setLang(lang) {
  localStorage.setItem('mv_lang', lang)
}
