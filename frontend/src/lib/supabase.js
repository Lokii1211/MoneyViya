...
function generateToken() {
  ...
  return 'viya_' + Array.from(arr).map(b => b.toString(16).padStart(2, '0')).join('')
}
...
export const api = {
  async login(phone, password) {
    try {
      ...
      if (user.password_hash !== hashed) return { success: false, message: 'Wrong password.' }
  ...
  async register(phone, password, name = 'User') {
    try {
      ...
      const hashed = await hashPassword(password, cleanPhone)
      await insert('users', { phone: cleanPhone, password_hash: hashed, name })
  ...
  async getUserReminders(phone) { return query('user_reminders', `?phone=eq.${phone}&select=*&order=time.asc`) },
  async createUserReminder(data) { return insert('user_reminders', data) },
  async updateUserReminder(id, data) { return update('user_reminders', `id=eq.${id}`, data) },
  async deleteUserReminder(id) { return remove('user_reminders', `id=eq.${id}`) },

  async getFamilyConnections(phone) { return query('family_connections', `?owner_phone=eq.${phone}&select=*&order=created_at.desc`) },
  async getFamilyInvitesReceived(phone) { return query('family_connections', `?member_phone=eq.${phone}&select=*&order=created_at.desc`) },
  async sendFamilyInvite(ownerPhone, memberPhone, relation) { return insert('family_connections', { owner_phone: ownerPhone, member_phone: memberPhone, relation, status: 'pending' }) },
  async respondFamilyInvite(id, status) { return update('family_connections', `id=eq.${id}`, { status }) },
  async removeFamilyConnection(id) { return remove('family_connections', `id=eq.${id}`) },
  async sendFriendRequest(ownerPhone, memberPhone) { return insert('family_connections', { owner_phone: ownerPhone, member_phone: memberPhone, relation: 'Friend', connection_type: 'friend', status: 'pending' }) },
